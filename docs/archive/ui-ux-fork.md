...
> **History, kept on purpose. Nothing here describes the plugin that ships.**
>
> This is the design the fork was being cut down to between 2026-08-11 and the
> rebuild. What runs is [`../ui-ux.md`](../ui-ux.md), and the shape underneath
> it is [`../architecture.md`](../architecture.md).
>
> What survives from this page: the words, the one-screen rule, the refusal of
> a folder picker, and the measured finding that Obsidian fires a `delete` for
> every descendant before the folder's own. What does not: the control plane,
> the relay, `status.py`, *can read* and *can edit* (there is one kind of person
> in a vault), the member screens, and the settings tab it describes.
