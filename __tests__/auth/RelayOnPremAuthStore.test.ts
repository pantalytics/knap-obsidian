...
describe("RelayOnPremAuthStore", () => {
	...
	describe("save() and load()", () => {
		...
		test("Load returns null if data incomplete (missing user)", () => {
			...
			const key = `synced-vaults_onprem_auth_${APP_ID}_${SERVER_ID}`;
	...
	describe("Storage key format", () => {
		test("P32: Key format synced-vaults_onprem_auth_{appId}_{server}", () => {
			store.save(SERVER_ID, mockAuthData);

			const expectedKey = `synced-vaults_onprem_auth_${APP_ID}_${SERVER_ID}`;
