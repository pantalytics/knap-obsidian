...
export class UpdateManager extends Observable<UpdateManager> {
	...
	constructor(
		...
		super("UpdateManager");
		this.githubReleases = new LocalStorage("evc-team-relay/releases");
		this.releaseChannels = new LocalStorage("evc-team-relay/releaseChannels");
	}
	...
	private async fetchReleases(): Promise<Release[]> {
		...
		try {
			...
			if (!response.ok) {
				// Silently fail for private repos or rate limiting
				if (response.status === 404 || response.status === 403) {
					this.debug(`GitHub API returned ${response.status}, skipping update check`);
					return [];
				}
			...
			// Silently handle network errors for update checks
			this.debug("Update check skipped:", error);
			return [];
		}
	}
