...
- The settings screen reads the same state, on a timer. Its status row was two variables set once by the sign-in flow and never touched again, so it said Syncing while the screen made the share and Up to date on every visit after that, whatever the vault was doing.
...
- A vault waiting on either of those reads Paused rather than Up to date, in the corner of the window as well as on the screen. Nothing is moving, and saying up to date over a vault that is syncing nothing is the same lie the entry above this one is about.
- The plugin shows how far it has got (#41). Adopting a vault of a few thousand notes is hours of work on this machine, and the only count was on Knap's web page, which reports the server's side of it. The count now sits beside the mark in the corner of the window and on the settings screen, with a bar, in the same words the web page uses: 290 of 2,567. Both come off this plugin's own queue. Neither appears once there is nothing left to carry.
